...
/*----Ajout fonctions */
function obtenirTotalFraisKM($unIdVisiteur, $mois){
$unIdVisiteur = filtrerChainePourBD($unIdVisiteur);
$requete = "SELECT(LigneFraisForfait.quantite * Vehicules.frais_km)
            FROM LigneFraisForfait
            INNER JOIN Visiteur ON LigneFraisForfait.idVisiteur = Visiteur.id
            INNER JOIN Vehicules ON Visiteur.vehicule_id = Vehicules.id
            WHERE idVisiteur ='" . $unIdVisiteur ."' AND idFraisForfait='km' AND mois='" . $mois ."'";
